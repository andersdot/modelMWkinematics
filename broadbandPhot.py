...
res = queryDatabase(query, columns, cuts, cache_file='gaiaunwisetmass.pkl')
...
good = ~np.isnan(tbl['w1_mag']-tbl['w2_mag']) & (np.abs(tbl['w1_mag'] - tbl['w2_mag']) < 0.5) & (tbl['j_m'] - tbl['k_m'] < 2.) & (tbl['w1_mag_err'] < 0.1) & (tbl['w2_mag_err'] < 0.1)
...
plt.ylabel('tmass j-k')
plt.xlabel('wise 1-2')
plt.savefig('tmasswiseCC_dustCorrected.png')
